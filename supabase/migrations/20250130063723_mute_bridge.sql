...
BEGIN
  ...
  WITH contact_data AS (
    SELECT 
      ...
      GREATEST(
        -- Exact name match
        CASE WHEN c.first_name || ' ' || c.last_name ILIKE p_query THEN 1.0::double precision
        -- Partial name match
        WHEN c.first_name || ' ' || c.last_name ILIKE '%' || p_query || '%' THEN 0.8::double precision
        -- Business name match
        WHEN c.business_name ILIKE '%' || p_query || '%' THEN 0.7::double precision
        -- Email match
        WHEN c.email ILIKE '%' || p_query || '%' THEN 0.6::double precision
